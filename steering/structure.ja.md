...
**Primary Pattern**: Monorepo with Library-First Architecture

> LabFlowはモノレポ構成を採用し、すべての機能を再利用可能なライブラリとして実装します。
> プラグインアーキテクチャにより、研究分野別の機能を独立したプラグインとして拡張可能です。
> - **packages/core**: 再利用可能なコアライブラリ
> - **packages/cli**: CLIインターフェース
> - **packages/web**: Webアプリケーション
> - **plugins/**: 研究分野別プラグイン（創薬、材料、気候、ゲノミクス）
...
│   ├── core/             # Core library (Article I: Library-First)
│   │   └── src/
│   │       ├── auth/         # Authentication providers
│   │       ├── db/           # Database schema (Drizzle)
│   │       ├── knowledge/    # RAG, document processing
│   │       ├── model-catalog/# AI model catalog & executor
│   │       ├── nli/          # Natural Language Interface
│   │       ├── plugin/       # Plugin system
│   │       ├── project/      # Project management
│   │       ├── screening/    # Screening pipelines
│   │       ├── visualization/# Data visualization
│   │       └── workflow/     # Workflow engine
│   ├── cli/              # CLI tool (Article II)
│   └── web/              # Next.js web application
├── plugins/              # Domain-specific plugins (planned)
├── References/           # Reference implementations
│   ├── graphrag/         # Microsoft GraphRAG
│   ├── esperanto/        # LLM unified interface
│   └── ...               # Other references
...
- **Article I**: Library-first pattern in `packages/core/`
- **Article II**: CLI interfaces in `packages/cli/`
...
|--------|--------|-------------|
| `auth/` | ✅ Implemented | Multi-provider authentication (Local, EntraID, Shibboleth) |
| `db/` | ✅ Implemented | Database schema with Drizzle ORM |
| `knowledge/` | ✅ Implemented | RAG service, document processor, vector store |
| `model-catalog/` | ✅ Implemented | AI model catalog and executor |
| `nli/` | ✅ Implemented | Natural Language Interface (JA/EN) |
| `plugin/` | ✅ Implemented | Plugin loader, manager, registry |
| `project/` | ✅ Implemented | Project service and wizard |
| `screening/` | ✅ Implemented | Primary, AI, Simulation screening |
| `visualization/` | 🔄 Partial | Data visualization utilities |
| `workflow/` | ✅ Implemented | Scientific workflow engine |

### NLI Service Features (DASH-NLI-*)

- **DASH-NLI-001**: Japanese/English input support with auto-detection
- **DASH-NLI-002**: Intent analysis with workflow recommendations
- **DASH-NLI-003**: Information extraction (domain, properties, data formats)
- **DASH-NLI-004**: Clarification question generation
- **DASH-NLI-007**: Input pattern recognition (predict/generate/optimize/analyze/compare)

### Screening Module Features

- **Primary Screening**: Rule-based filtering (drug discovery, materials filters)
- **AI Screening**: ML model predictions with uncertainty estimation
- **Simulation Screening**: Docking, MD, FEP, DFT, Phonon simulations
...
- 実装済みコアモジュールの状況を追加
- NLI/Screening機能の詳細を追記
- ディレクトリ構造を実際の実装に合わせて更新

### Version 1.1 (2025-12-13)

- アーキテクチャパターンをMonorepo with Library-Firstに確定
- プラグインアーキテクチャの追加

---

**Last Updated**: 2025-12-14
**Maintained By**: LabFlow Team
